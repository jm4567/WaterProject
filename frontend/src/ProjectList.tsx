...
function ProjectList({ selectedCategories }: { selectedCategories: string[] }) {
  //remember it does not know the selectedCategories in App.tsx
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      const categoryParams = selectedCategories
        ...
        .join('&'); //makes sure everything is set up correctly
      const response = await fetch(
        `https://localhost:5000/api/Water/AllProjects?pageHowMany=${pageSize}&pageNum=${pageNum}${selectedCategories.length ? `&${categoryParams}` : ''}`,
        {
          credentials: 'include',
        }
      );
  ...
  }, [pageSize, pageNum, totalItems, selectedCategories]); //dependency array
  return (
    <>
      {projects.map((p) => (
